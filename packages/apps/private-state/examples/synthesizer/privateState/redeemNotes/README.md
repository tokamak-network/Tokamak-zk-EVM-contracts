...
Use the generated static input folders and the stored transaction snapshots created by the integrating repository's `packages/apps/private-state` deployment flow. Each `redeemNotes*` folder contains the four JSON files consumed directly by `tokamak-ch-tx`:
