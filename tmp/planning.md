...
|---|---|---|
| Official Public Observer | `packages/apps/private-state/cli/lib/runtime.mjs` hardcodes `https://observer.tonnel.io`; monitoring docs identify the same deployed observer URL. | Treat as an official hosted surface. The Privacy Notice must cover hosting/CDN logs, request metadata, public blockchain and Channel records displayed through the observer, retention, and any access logs available to Provider Parties. Hosting provider, CDN, storage, logging, and retention details remain to be confirmed. |
...
| GitHub issues and repository support | CLI `package.json` uses `https://github.com/tokamak-network/Tokamak-zk-EVM-contracts/issues` as the bug-report URL. No separate support email, Discord, Telegram, or form was found in the audited paths. | Treat GitHub issues as a support/contact channel that may include user-submitted wallet addresses, transaction hashes, logs, screenshots, and contact identifiers. Record that no other official support channel is currently evidenced in the repository; confirm before final notice. |
| CLI package distribution | CLI `package.json` publishes `@tokamak-private-dapps/private-state-cli` to the public npm registry. The CLI also checks the npm registry for latest private-state CLI metadata. | Treat npm as a Third-Party Service for package install/update metadata, download logs, and account-level registry data. Confirm whether Provider Parties can access npm package download analytics or account metadata. |
| Public deployment artifact distribution | Root README states that bridge deployment artifacts and DApp registration artifacts are published to Google Drive. `packages/common/src/artifact-cache.mjs` downloads the public Drive artifact index and files. | Treat Google Drive as a Third-Party Service for artifact access/download metadata. The notice should disclose artifact-download requests and Drive-managed cookies or confirmation tokens where applicable. Confirm whether Provider Parties can access Drive download, access, or account logs. |
| Public CRS and proof-runtime artifact distribution | CLI README requires access to the public Groth16 CRS archive source and install mode downloads proof/runtime artifacts. | Treat the CRS archive host and proof-runtime package sources as Third-Party Services. Identify exact hosts, account owners, retention, and whether Provider Parties can access logs before final notice. |
| User-selected RPC providers | CLI docs require `set rpc`; built-in presets and guidance mention Alchemy, Ankr, Chainstack, Chainnodes, and QuickNode. The user supplies the RPC URL. | Treat ordinary RPC providers as user-selected Third-Party Services unless Provider Parties operate, proxy, or recommend a specific endpoint as an official service. The notice should explain that RPC providers can see request metadata and public blockchain queries. Confirm whether any Provider-operated or proxied RPC endpoint exists. |
| Workspace mirror hosts | CLI supports Channel workspace mirror recovery from registered mirror URLs. Mirror URLs can be operated by Channel Operators or other parties. | Treat mirror hosts as Channel Operator or Third-Party Services unless Provider Parties operate the mirror. The notice should cover that mirror operators may receive request metadata during mirror recovery. Confirm whether The Great First Channel has an official mirror and who operates it. |
...
| Analytics, cookies, telemetry, error reporting, uptime monitoring, and abuse-protection tools | No private-state CLI or documentation evidence was found for first-party CLI telemetry, analytics SDKs, Sentry, PostHog, Segment, Mixpanel, Plausible, Google Analytics, or similar tooling in the audited paths. Google Drive download code handles Drive-provided cookies or confirmation tokens for third-party artifact downloads. | Do not claim that no analytics, telemetry, cookies, logging, or monitoring exists across the whole Service until hosting configuration is confirmed. Record current repository evidence as none for CLI telemetry and first-party analytics, with hosting/CDN/logging details still open. |
