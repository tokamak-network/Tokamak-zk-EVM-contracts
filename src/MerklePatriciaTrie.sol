...
contract MerklePatriciaTrie {
    ...
    bytes32 constant public EMPTY_TRIE_HASH = 0x56e81f171bcc55a6ff8345e692c0f86e5b48e01b996cadc001622fb5e363b421;
    ...
    uint8 constant private HP_EXTENSION_EVEN = 0x00;
    uint8 constant private HP_EXTENSION_ODD = 0x10;
    uint8 constant private HP_LEAF_EVEN = 0x20;
    uint8 constant private HP_LEAF_ODD = 0x30;
    ...
    function _hexPrefix(bytes memory nibbles, bool isLeaf) private pure returns (bytes memory) {
        ...
        if (nibbles.length % 2 == 1) {
            ...
            for (uint i = 1; i < nibbles.length; i += 2) {
            ...
            for (uint i = 0; i < nibbles.length; i += 2) {
                encoded[i / 2 + 1] = bytes1((uint8(nibbles[i]) << 4) | uint8(nibbles[i + 1]));
            }
            return encoded;
        }
    ...
    function _insert(
        ...
        if (nodeList.length == 2) {
            ...
            bytes32 childRef = nodeList[branchKey].length > 0 
                ? abi.decode(nodeList[branchKey], (bytes32)) 
                : bytes32(0);
     ...
     */
    function _matchingNibbleLength(
        bytes memory a,
        uint256 aOffset,
        bytes memory b
