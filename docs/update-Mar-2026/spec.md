...
- $\mathcal{R}\subseteq\mathrm{AppStorageAddrs}\times\mathrm{StateIndices}\times\mathrm{VerifiedStateRoots}$
  ...
  - Setter-gated root update: $\forall s\in\mathrm{AppStorageAddrs},\ \forall t\in\mathrm{StateIndices},\ \forall r_t,\mathrm{updatedRoot}\in\mathrm{VerifiedStateRoots},\ \left((t<\mathrm{stateIndex}\wedge(s,t,r_t)\in\mathcal{R}\wedge(s,t+1,\mathrm{updatedRoot})\in\mathcal{R})\Rightarrow \exists k\in\mathrm{AppUserStorageKeys},\ \exists \mathrm{updatedStorageValue}\in\mathbb{F}_{256},\ \mathrm{updateStorage}(s,k,\mathrm{updatedStorageValue},\mathrm{updatedRoot})=\mathrm{true}\right)$
