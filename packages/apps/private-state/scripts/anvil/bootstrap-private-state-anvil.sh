...
# The anvil bootstrap flow must not depend on packages/apps/.env being present or set to an anvil network.
...
APPS_ENV_FILE="$TEMP_ENV_FILE" bash "$PROJECT_ROOT/packages/apps/private-state/scripts/deploy/deploy-private-state.sh"
APPS_ENV_FILE="$TEMP_ENV_FILE" bash "$PROJECT_ROOT/packages/apps/private-state/scripts/deploy/write-deploy-artifacts.sh" "$APPS_CHAIN_ID"
