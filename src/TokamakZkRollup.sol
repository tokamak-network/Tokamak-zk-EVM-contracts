...
library EdDSA {
    // Simplified EdDSA signature structure 
    struct Signature {
        uint256 R_x;
        uint256 R_y;
        uint256 s;
...
contract ZKRollupBridge is ReentrancyGuard {
    ...
    function assignL2Address(
        address l2Address,
        EdDSA.PublicKey calldata l2PublicKey
    ) external {
    ...
    function openChannel(
        ...
        EdDSA.PublicKey[] calldata publicKeys,
        bytes32 preprocessCommitment,
        uint256 timeout
        ...
        channel.preprocessCommitment = preprocessCommitment;
        ...
        for (uint256 i = 0; i < participants.length; i++) {
            ...
            if (users[participant].l2PublicKey.x == 0) {
    ...
    function depositETH(uint256 channelId) external payable nonReentrant {
        Channel storage channel = channels[channelId];
        require(channel.state == ChannelState.Initialized || channel.state == ChannelState.Open, "Invalid channel state");
        require(channel.isParticipant[msg.sender], "Not a participant");
        require(msg.value > 0, "Must deposit something");
        
        channel.participantDeposits[msg.sender] += msg.value;
        
        emit Deposited(channelId, msg.sender, address(0), msg.value);
    }
    
    function depositToken(uint256 channelId, address token, uint256 amount) external nonReentrant {
        Channel storage channel = channels[channelId];
        require(channel.state == ChannelState.Initialized || channel.state == ChannelState.Open, "Invalid channel state");
        require(channel.isParticipant[msg.sender], "Not a participant");
        require(amount > 0, "Must deposit something");
        require(token == channel.targetContract, "Token must be target contract");
        
        // Transfer tokens from user to this contract
        IERC20(token).transferFrom(msg.sender, address(this), amount);
        
        channel.tokenDeposits[token][msg.sender] += amount;
    ...
    function withdrawAfterClose(uint256 channelId) external nonReentrant {
        ...
        uint256 ethAmount = channel.finalBalances[msg.sender];
        if (ethAmount > 0) {
            channel.finalBalances[msg.sender] = 0;
            (bool success, ) = msg.sender.call{value: ethAmount}("");
            require(success, "ETH transfer failed");
            emit Withdrawn(channelId, msg.sender, address(0), ethAmount);
        }
    ...
    function emergencyWithdraw(uint256 channelId) external nonReentrant {
        Channel storage channel = channels[channelId];
        require(
            block.timestamp >= channel.openTimestamp + channel.timeout + LOCK_TIMEOUT,
    ...
    function lockStateAndConvert(
        uint256 channelId,
        bytes calldata multiSigSignature
        ...
        bytes32 lockMessage = keccak256(abi.encodePacked(
            "LOCK_STATE",
            channelId,
    ...
    function signAggregatedProof(
        uint256 channelId,
        EdDSA.Signature calldata signature
    ) external {
        Channel storage channel = channels[channelId];
    ...
    function closeChannel(
        ...
        require(IVerifier.verify(a, b, c, publicInputs), "Invalid ZK proof");
        
        // Verify public inputs match the channel state
        require(
            keccak256(abi.encodePacked(publicInputs)) == channel.aggregatedProofHash,
            "Proof mismatch"
        );
    ...
    function _updateL1State(uint256 channelId) internal {
        ...
        for (uint256 i = 0; i < channel.participants.length; i++) {
            ...
            if (user.isLocked) {
                ...
                channel.finalBalances[participant] = channel.participantDeposits[participant];
    ...
    function _verifyEdDSA(
        bytes32 message,
        EdDSA.Signature calldata signature,
        EdDSA.PublicKey storage publicKey
    ...
    function getChannelInfo(uint256 channelId) external view returns (
        ...
        return (
            ...
            channel.zkMerkleRoots
        );
    }
    
    function getUserLockStatus(address user) external view returns (
        bool isLocked,
        uint256 lockedAmount,
        uint256 lockTimestamp
    ) {
