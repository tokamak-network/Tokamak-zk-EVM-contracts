...
- $\widetilde{\mathcal{K}}:=\bigcup_{c\in\mathrm{ChannelIds}}\left(\{c\}\times\mathcal{K}_c\right)$
  - Assumption (cross-user key separation on each app storage): $\forall c\in\mathrm{ChannelIds},\ \forall s\in\mathrm{AppStorageAddrs}_c,\ \forall u_1,u_2\in\mathrm{UserAddrs}_c,\ \forall k_1,k_2\in\mathrm{UserChannelStorageKeys}_c,\ \left(((c,u_1,s,k_1)\in\widetilde{\mathcal{K}}\wedge(c,u_2,s,k_2)\in\widetilde{\mathcal{K}}\wedge u_1\neq u_2)\Rightarrow k_1\neq k_2\right)$
