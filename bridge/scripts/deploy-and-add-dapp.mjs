...
function usage() {
  console.log(`Usage:
  node bridge/scripts/deploy-and-add-dapp.mjs --network <anvil|sepolia|mainnet> --group <example-group> [--group <example-group> ...] --dapp-id <uint> [options]

This orchestrator deploys the private-state app first and then invokes:
  node bridge/scripts/admin-add-dapp.mjs ...

Deploy-only options:
  --network <name>                  Bridge network used by admin-add-dapp.mjs
  --app-network <name>              App deployment network; defaults to --network
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    switch (current) {
      case "--network":
        deployOptions.network = take(current);
        forwardedArgs.push(current, deployOptions.network);
        break;
      case "--app-network":
...
async function main() {
  ...
  resolveAppNetwork(deployOptions.network);
  const appNetwork = deployOptions.appNetwork ?? deployOptions.network;
  ...
  await run("node", [deployPrivateStateScriptPath, "--network", appNetwork], {
    cwd: repoRoot,
    env: deployEnv,
  });
