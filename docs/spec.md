...
- $\mathcal{R}\subseteq\mathrm{StateIndices}\times\mathrm{AppStorageAddrs}\times\mathrm{VerifiedStateRoots}$
  - Existence and uniqueness per storage-index pair (conditional on index realization): $\forall t\in\mathrm{StateIndices},\ \left(\left(\exists s_0\in\mathrm{AppStorageAddrs},\ \exists r_0\in\mathrm{VerifiedStateRoots},\ (t,s_0,r_0)\in\mathcal{R}\right)\Rightarrow\left(\forall s\in\mathrm{AppStorageAddrs},\ \exists!r\in\mathrm{VerifiedStateRoots},\ (t,s,r)\in\mathcal{R}\right)\right)$
  ...
  - State transition by one-step index increment with root update (conditional on adjacent index realization): $\forall t\in\mathrm{StateIndices},\ \left(\left(\exists s_t\in\mathrm{AppStorageAddrs},\ \exists r_t\in\mathrm{VerifiedStateRoots},\ (t,s_t,r_t)\in\mathcal{R}\ \wedge\ \exists s_{t+1}\in\mathrm{AppStorageAddrs},\ \exists r_{t+1}\in\mathrm{VerifiedStateRoots},\ (t+1,s_{t+1},r_{t+1})\in\mathcal{R}\right)\Rightarrow\exists s\in\mathrm{AppStorageAddrs},\ \exists r_t^\prime,r_{t+1}^\prime\in\mathrm{VerifiedStateRoots},\ (t,s,r_t^\prime)\in\mathcal{R}\wedge(t+1,s,r_{t+1}^\prime)\in\mathcal{R}\wedge r_t^\prime\neq r_{t+1}^\prime\right)$
