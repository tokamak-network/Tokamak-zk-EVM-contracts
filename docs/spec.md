...
- $\mathcal{V}\subseteq\mathrm{AppStorageAddrs}\times\mathrm{UserChannelStorageKeys}\times\mathrm{ValidatedStorageValues}$
  ...
  - Verified-commit synchronized update:
    ...
    &\mathrm{verifyProposedStateRoots}(\mathrm{forkId},\mathrm{unverifiedStateIndex},\mathrm{appStorageAddrs},\mathrm{userChannelStorageKeys},\mathrm{updatedStorageValues},\mathrm{updatedRoots},\mathrm{proofTokamak},\mathrm{preprocessTokamak},\mathrm{publicInputTokamak})=\mathrm{true}\\
...
- $\mathcal{R}\subseteq\mathrm{StateIndices}\times\mathrm{AppStorageAddrs}\times\mathrm{VerifiedStateRoots}$
  ...
  - State transition by one-step index increment with root update (non-trivial adjacent change): $\forall t\in\mathrm{StateIndices},\ \left(\left(\exists s\in\mathrm{AppStorageAddrs},\ \exists r\in\mathrm{VerifiedStateRoots},\ (t,s,r)\in\mathcal{R}\right)\wedge\left(\exists s^\prime\in\mathrm{AppStorageAddrs},\ \exists r^\prime\in\mathrm{VerifiedStateRoots},\ (t+1,s^\prime,r^\prime)\in\mathcal{R}\right)\Rightarrow\exists s^{\ast}\in\mathrm{AppStorageAddrs},\ \exists r_t,r_{t+1}\in\mathrm{VerifiedStateRoots},\ (t,s^{\ast},r_t)\in\mathcal{R}\wedge(t+1,s^{\ast},r_{t+1})\in\mathcal{R}\wedge r_t\neq r_{t+1}\right)$
...
- $\mathcal{N}\subseteq\mathrm{ForkIds}\times\mathrm{StateIndices}\times\mathrm{AppStorageAddrs}\times\mathrm{ProposedStateRoots}$
  ...
  - State transition by one-step index increment with root update (non-trivial adjacent change): $\forall f\in\mathrm{ForkIds},\ \forall t\in\mathrm{StateIndices},\ \left(\left(\exists s\in\mathrm{AppStorageAddrs},\ \exists r\in\mathrm{ProposedStateRoots},\ (f,t,s,r)\in\mathcal{N}\right)\wedge\left(\exists s^\prime\in\mathrm{AppStorageAddrs},\ \exists r^\prime\in\mathrm{ProposedStateRoots},\ (f,t+1,s^\prime,r^\prime)\in\mathcal{N}\right)\Rightarrow\exists s^{\ast}\in\mathrm{AppStorageAddrs},\ \exists r_t,r_{t+1}\in\mathrm{ProposedStateRoots},\ (f,t,s^{\ast},r_t)\in\mathcal{N}\wedge(f,t+1,s^{\ast},r_{t+1})\in\mathcal{N}\wedge r_t\neq r_{t+1}\right)$
