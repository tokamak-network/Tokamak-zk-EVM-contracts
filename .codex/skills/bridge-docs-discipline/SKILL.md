...
description: Use when editing `docs/spec.md`, `docs/zk-l2-bridge-design-notes.md`, or adjacent bridge documentation in this repository. It enforces the role split that `spec.md` is a math-first abstract model for theory and paper work, while `zk-l2-bridge-design-notes.md` is a stable high-level architecture document for future developer docs and should avoid volatile implementation detail.
---
...
Treat `docs/spec.md` as the abstract mathematical model.
...
Treat `docs/zk-l2-bridge-design-notes.md` as a stable architecture note for future
...
3. Put each fact in exactly one document family:
   - abstract theorem/spec fact -> `docs/spec.md`
   - stable design fact -> `docs/zk-l2-bridge-design-notes.md`
   - current implementation fact -> `docs/current-implementation.md`
...
5. If a statement is likely to change after a small refactor, it usually does not
   belong in `docs/zk-l2-bridge-design-notes.md`.
