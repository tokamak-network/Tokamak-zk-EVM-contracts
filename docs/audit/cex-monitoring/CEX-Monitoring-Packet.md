...
The private-state DApp includes a static browser investigator at
`packages/apps/private-state/investigator/index.html`. It is not generated by the Monitoring Packet
script. Users can load a local raw evidence ZIP created by `wallet get-notes --export-evidence`,
