...
|---|---|---|
| Official Public Observer | `packages/apps/private-state/cli/lib/runtime.mjs` hardcodes `https://observer.tonnel.io`; monitoring docs identify the same deployed observer URL. The separate `/Users/jehyuk/repo/channel-workspace-mirror/` repository shows that the observer is a Next.js app configured for Vercel, uses `@vercel/analytics`, serves public mirror and observer APIs through Vercel, stores indexed observer and mirror metadata in Neon, stores large mirror artifacts in Vercel Blob, and uses an EC2 worker for persistent CLI recovery, raw RPC history, observer sync, and mirror publishing. | Treat as an official hosted surface operated by the individual Provider. The Privacy Notice must cover Vercel hosting/runtime request metadata, Vercel Analytics page-view data, Vercel Blob artifact access, Neon-stored public observer and mirror metadata, EC2 worker operational logs, public blockchain and Channel records displayed through the observer, and any logs available to the Provider. Exact Vercel plan, runtime-log retention, analytics retention, Neon retention, Blob retention, EC2 journal retention, and whether Vercel Log Drains are enabled remain to be confirmed from deployment settings. |
| Official documentation and repository pages | Root and CLI READMEs point users to GitHub-hosted documentation and repository pages. | Treat GitHub and any documentation host as Third-Party Services unless Provider Parties operate a separate official website. Confirm whether any non-GitHub official website or docs host exists. |
...
| CLI package distribution | CLI `package.json` publishes `@tokamak-private-dapps/private-state-cli` to the public npm registry. The CLI also checks the npm registry for latest private-state CLI metadata. Public npm APIs confirm package metadata, public maintainer metadata, latest version metadata, and aggregate public download counts by period. | Treat npm as a Third-Party Service for package install/update metadata, download logs, and account-level registry data. The notice should disclose Provider access to npm package analytics or owner metadata to the extent npm makes it available to the Provider. Private owner-visible npm analytics fields and retention remain npm account or product-feature confirmations. |
| Public deployment artifact distribution | Root README states that bridge deployment artifacts and DApp registration artifacts are published to Google Drive. `packages/common/src/artifact-cache.mjs` downloads the public Drive artifact index and files. The Provider states that, if Google Drive supports artifact folder or download-log access, the individual Provider can access that information. | Treat Google Drive as a Third-Party Service for artifact access/download metadata. The notice should disclose artifact-download requests, Drive-managed cookies or confirmation tokens, and Provider access to folder or download information to the extent Google Drive makes it available to the Provider. Confirm the exact Google Drive data fields and retention before final notice. |
| Public CRS and proof-runtime artifact distribution | CLI README requires access to the public Groth16 CRS archive source and install mode downloads proof/runtime artifacts. `packages/groth16/lib/public-drive-crs.mjs` confirms that the public Groth16 MPC CRS archive source is a Provider-operated artifact publication surface hosted on Google Drive. | Treat the CRS/proof-runtime artifact publication surface as Provider-operated and Google Drive-hosted. The Privacy Notice must cover request metadata, artifact paths, timestamps, IP address/user agent if logged by Google Drive, retention, storage location, and Google Drive as the hosting provider. Google Drive owner-visible download log fields, retention, and deletion/access route remain Google account or product-feature confirmations. |
...
| Analytics, cookies, telemetry, error reporting, uptime monitoring, and abuse-protection tools | No private-state CLI telemetry was found in the audited CLI paths. The separate observer repository imports `@vercel/analytics/next` in `app/layout.tsx`, so the observer uses Vercel Web Analytics. Google Drive download code handles Drive-provided cookies or confirmation tokens for third-party artifact downloads. | State that the CLI itself has no current first-party telemetry evidence, but the official observer uses Vercel Web Analytics. The notice must cover Vercel Analytics, Vercel runtime logs, Drive-managed cookies or confirmation tokens, and any hosting/CDN/security monitoring configured in deployment settings. Do not claim that no analytics, telemetry, cookies, logging, or monitoring exists across the whole Service until hosting configuration is confirmed. |
...
- The Provider's residential address is not published. If a physical notice address becomes legally or operationally
  ...
  or counsel address.
- Google Drive artifact-folder and download-log information is Provider-accessible to the extent Google Drive makes that
  information available to the Provider.
- npm package download analytics or owner metadata is Provider-accessible to the extent npm makes that information
...
- The public CRS/proof-runtime artifact publication surface is operated by the individual Provider through Google Drive.
  Google Drive logs or download information are Provider-accessible to the extent Google Drive makes that information
  available to the Provider.
...
Verification notes for items that previously required confirmation:
...
- Vercel's public runtime-log documentation lists dashboard-visible request details such as method, path, timestamp,
  ...
  and outgoing request data. The same documentation states retention by plan: Hobby 1 hour, Pro 1 day, Enterprise 3
  days, and Observability Plus 30 days. Exact project plan, enabled Log Drains, and account-level analytics retention
  remain deployment-setting confirmations.
...
- Large mirror artifacts are stored in Vercel Blob and served through redirects from the Vercel app. Exact Blob region,
  object retention, and deletion policy remain deployment-setting confirmations.
- The persistent worker is an EC2/systemd worker. It stores the private-state CLI workspace and raw RPC history on the
  ...
  configured. Exact AWS region, journal retention, raw-history retention, EBS backup policy, and operational deletion
  route remain host-setting confirmations.
...
- The public Groth16 MPC CRS archive source is a Provider-operated artifact publication surface hosted on Google Drive.
  ...
  `1jAIBqV-KG6PxFPDFpgtg9PDIceDDqk6N` and downloads matching archive files by Drive file ID. Exact Google Drive
  owner-visible download log fields and retention remain Google account or Google Drive feature confirmations.
- The public npm registry confirms `@tokamak-private-dapps/private-state-cli` package metadata and public download-count
  ...
  npm's public download-count API exposes aggregate package downloads by period, not individual downloader logs. Any
  private owner-visible npm analytics, package-owner metadata, and retention remain npm account or npm product-feature
  confirmations.
...
| Observer worker operational RPC | Runtime RPC URL and scan parameters stored in Neon `indexer_runtime_config`, observer `eth_getLogs` and `eth_getBlockByNumber` requests, raw RPC history stored on the EC2 worker, sync state, indexer errors, and last-run status. | Operate the Official Public Observer and workspace mirror for The Great First Channel. | Neon runtime rows have no repository-managed deletion policy. EC2 raw-history and journal retention remain host-setting confirmations. | Neon stores runtime config and sync state; the configured RPC provider processes observer sync requests; AWS EC2 stores worker state. | This is Provider-controlled infrastructure data. Requests can be sent to `cjhyuck213@gmail.com`, subject to public-record, security, operational, and legal limits. |
...
| Official documentation and source pages | Page request metadata, GitHub account data if signed in, documentation paths viewed, and any comments or issue content submitted by users. | Provide documentation, source references, release notes, and support links. | Retention is controlled by GitHub or any other documentation host used. No non-GitHub official documentation host is confirmed in this planning pass. | GitHub is the confirmed Third-Party Service for repository documentation and issues. | Users must use GitHub's privacy route for GitHub-controlled data. Provider-controlled contact remains `cjhyuck213@gmail.com`. |
...
- Draft the Privacy Notice from the matrix above. The draft must clearly separate Provider-controlled processing from
  ...
  keep still-unconfirmed deployment/account settings as operational confirmations rather than legal promises.
