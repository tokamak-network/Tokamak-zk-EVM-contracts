...
contract ArchitecturalOptimizationTest is Test {
    ...
    function setUp() public {
        ...
        bytes memory originalInitData = abi.encodeCall(
            RollupBridgeV1.initialize,
            (address(verifier), address(originalMtmanager), owner)
        );
    ...
    function testScalabilityWith40Users() public {
        ...
        bytes memory freshOriginalInitData = abi.encodeCall(
            RollupBridgeV1.initialize,
            (address(freshVerifier), address(freshOriginalMtmanager), owner)
        );
        ...
        bytes memory freshRedesignedInitData = abi.encodeCall(
            RollupBridgeV2.initialize,
            (address(freshVerifier), address(0), owner)
        );
        ...
        // Verify functional equivalence
        (,, uint256 originalParticipantCount, bytes32 originalRoot,) = freshOriginalBridge.getChannelInfo(originalChannelId);
        (,, uint256 redesignedParticipantCount, bytes32 redesignedRoot,) = freshRedesignedBridge.getChannelInfo(redesignedChannelId);
