...
1. Duplicate artifact upload flow in bridge scripts
   - Files: `bridge/scripts/upload-bridge-artifacts.mjs`, `bridge/scripts/upload-dapp-artifacts.mjs`
   - Finding: Both scripts implement the same high-level upload flow: usage output, manual argument parsing, local/anvil upload skipping, Drive preflight, exclusive Drive folder creation, file upload, artifact index update, optional receipt writing, and final console output.
   - Recommendation: Extract a shared upload runner or shared option helpers under `scripts/drive/lib/` or `bridge/scripts/lib/`, while keeping bridge-specific and DApp-specific file collection separate.

2. Duplicate private-state E2E helper logic
   - Files: `packages/apps/private-state/scripts/e2e/run-bridge-private-state-e2e.mjs`, `packages/apps/private-state/scripts/e2e/run-bridge-private-state-cli-e2e.mjs`
   - Finding: The two harnesses share helper logic for argument parsing, command execution, JSON read/write, byte normalization, BigInt serialization, Tokamak transaction snapshots, artifact copying, and installed Tokamak setup checks.
   - Recommendation: Extract only stable helpers into a small `scripts/e2e/lib/` module. The scenario bodies should remain separate because one tests the lower-level genesis flow and the other tests the user-facing CLI flow.

3. Duplicate Groth16 MPC setup utility logic
...
4. Large inline Node blocks inside bridge deployment shell script
...
5. Manual provenance verification script has low discoverability
...
6. Groth16 trusted-setup path is retained for compatibility
