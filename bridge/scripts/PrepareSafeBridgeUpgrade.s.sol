...
contract PrepareSafeBridgeUpgradeScript is Script {
    using stdJson for string;

    struct SafeUpgradePlan {
        address owner;
        address deployer;
        address dAppManager;
    ...
    function run() external returns (SafeUpgradePlan memory result) {
        ...
        _writePlanArtifact(result, existingJson, outputPath);
        _logPlan(result);
    }

    function _writePlanArtifact(SafeUpgradePlan memory result, string memory existingJson, string memory outputPath)
        private
    {
        string memory deploymentJson = "bridgeSafeUpgradePlan";
        vm.serializeBool(deploymentJson, "safeUpgradePlan", true);
        ...
        if (existingJson.parseRaw(".abiManifestPath").length != 0) {
            vm.serializeString(deploymentJson, "previousAbiManifestPath", existingJson.readString(".abiManifestPath"));
        }
    ...
    function _logPlan(SafeUpgradePlan memory result) private view {
        console2.log("Bridge deployer:", result.deployer);
        console2.log("Bridge Safe owner:", result.owner);
