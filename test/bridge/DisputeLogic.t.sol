...
contract DisputeLogicTest is RollupBridgeTest {
    ...
    event ParticipantSlashed(
        uint256 indexed channelId, address indexed participant, uint256 slashAmount, string reason
    );
    ...
    function testLeaderBondSlashingForInvalidProof() public {
        ...
        vm.mockCall(address(verifier), abi.encodeWithSelector(IVerifier.verify.selector), abi.encode(false));
    ...
    function testEmergencyCloseExpiredChannelSlashesLeaderAndEnablesWithdrawals() public {
        ...
        assertEq(
            user1BalanceAfter, user1BalanceBefore + user1DepositBefore, "User should receive their original deposit"
        );
    ...
    function testDisputeChallengePeriodEnforcement() public {
        ...
        vm.prank(user1);
        uint256 disputeId = bridge.raiseDispute(channelId, "", "Test dispute within challenge period");
        ...
        bridge.raiseDispute(channelId, "", "Test dispute after challenge period");
    }
    ...
    function testDisputeRequiresClosedChannel() public {
        ...
        bridge.raiseDispute(channelId, "", "Test dispute on non-closed channel");
    }
    ...
    function testOnlyParticipantsCanRaiseDisputes() public {
        ...
        bridge.raiseDispute(channelId, "", "Non-participant trying to dispute");

        // Test 2: The leader is not a participant in this test setup, so they also cannot raise disputes
        vm.prank(leader);
        vm.expectRevert("Not a participant");
        bridge.raiseDispute(channelId, "", "Leader trying to dispute (but leader is not a participant in test setup)");
    }
    ...
    function testLeaderCannotReclaimBondAfterResolvedDispute() public {
        ...
        vm.prank(user1);
        uint256 disputeId = bridge.raiseDispute(channelId, "evidence", "Leader submitted invalid proof");
    ...
    function testLeaderCanReclaimBondWithoutResolvedDisputes() public {
        ...
        assertFalse(
            bridge.hasResolvedDisputesAgainstLeader(channelId), "Should have no resolved disputes against leader"
        );
        ...
        assertEq(
            leaderBalanceAfter, leaderBalanceBefore + bridge.LEADER_BOND_REQUIRED(), "Leader should receive bond back"
        );
    }
    ...
    function testLeaderCanReclaimBondAfterDisputeRejected() public {
        ...
        vm.prank(user1);
        uint256 disputeId = bridge.raiseDispute(channelId, "fake evidence", "Frivolous claim against leader");
    ...
    function testDisputeResolutionAutomaticallyEnablesEmergencyMode() public {
        ...
        // Raise a dispute against the leader
        vm.prank(user1);
        uint256 disputeId = bridge.raiseDispute(channelId, "evidence of misconduct", "Leader submitted invalid proof");

        // Emergency mode should still not be enabled (dispute is pending)
        assertFalse(
            bridge.isEmergencyModeEnabled(channelId), "Emergency mode should not be enabled while dispute is pending"
        );
        ...
        assertTrue(
            bridge.isEmergencyModeEnabled(channelId),
            "Emergency mode should be automatically enabled after dispute resolution"
        );
    ...
    function testDisputeRejectionDoesNotEnableEmergencyMode() public {
        ...
        // Raise a dispute against the leader
        vm.prank(user1);
        uint256 disputeId = bridge.raiseDispute(channelId, "false evidence", "Frivolous claim against leader");
        ...
        assertFalse(
            bridge.isEmergencyModeEnabled(channelId), "Emergency mode should not be enabled when dispute is rejected"
        );
    ...
    function testDisputeTimeoutAllowsFinalization() public {
        ...
        assertEq(
            uint8(dispute.status), uint8(DisputeLogic.DisputeStatus.Raised), "Dispute should still be in Raised state"
        );
