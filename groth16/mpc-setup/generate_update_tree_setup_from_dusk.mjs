...
import * as binFileUtils from "../circuits/node_modules/@iden3/binfileutils/build/main.cjs";
import { getCurveFromName } from "../circuits/node_modules/snarkjs/src/curves.js";
import { writeContributions, writePTauHeader } from "../circuits/node_modules/snarkjs/src/powersoftau_utils.js";
...
const outputDir = path.join(trustedSetupRoot, "updateTree");
...
async function createTruncatedPhase1PtauFromDuskResponse(responsePath, outputPath, targetPower) {
    const curve = await getCurveFromName("bls12-381");
    try {
        const expectedResponseSize =
            64 +
            ((2 ** duskSource.maxPower) * 2 - 1) * curve.G1.F.n8 +
            (2 ** duskSource.maxPower) * curve.G2.F.n8 +
            (2 ** duskSource.maxPower) * curve.G1.F.n8 +
            (2 ** duskSource.maxPower) * curve.G1.F.n8 +
            curve.G2.F.n8 +
            curve.G1.F.n8 * 2 * 6 +
            curve.G2.F.n8 * 2 * 3;
        const actualResponseSize = fs.statSync(responsePath).size;
        if (actualResponseSize !== expectedResponseSize) {
            throw new Error(
                `Unexpected Dusk response size. expected=${expectedResponseSize} actual=${actualResponseSize}`
            );
        }

        const fdResponse = fs.openSync(responsePath, "r");
        const fdPtau = await binFileUtils.createBinFile(outputPath, "ptau", 1, 7);
        let position = 64;

        try {
            await writePTauHeader(fdPtau, curve, targetPower);

            const sections = [
                {
                    sectionId: 2,
                    group: curve.G1,
                    compressedBytes: curve.G1.F.n8,
                    totalPoints: 2 ** (duskSource.maxPower + 1) - 1,
                    keptPoints: 2 ** (targetPower + 1) - 1
                },
                {
                    sectionId: 3,
                    group: curve.G2,
                    compressedBytes: curve.G2.F.n8,
                    totalPoints: 2 ** duskSource.maxPower,
                    keptPoints: 2 ** targetPower
                },
                {
                    sectionId: 4,
                    group: curve.G1,
            ...
            for (const section of sections) {
                position = await writeSectionFromResponse(fdResponse, fdPtau, position, section);
            }

            await writeContributions(fdPtau, curve, []);
        } finally {
...
async function writeSectionFromResponse(fdResponse, fdPtau, startPosition, section) {
    const encodedBytes = section.group.F.n8 * 2;
    const maxPointsPerChunk = 512;

    await binFileUtils.startWriteSection(fdPtau, section.sectionId);

    let position = startPosition;
    let writtenPoints = 0;
    while (writtenPoints < section.keptPoints) {
        const chunkPoints = Math.min(section.keptPoints - writtenPoints, maxPointsPerChunk);
        const compressed = readBufferSlice(
            fdResponse,
            position,
            chunkPoints * section.compressedBytes
        );
        const affine = new Uint8Array(chunkPoints * encodedBytes);
        for (let i = 0; i < chunkPoints; i += 1) {
            const point = section.group.fromRprCompressed(compressed, i * section.compressedBytes);
            section.group.toRprLEM(affine, i * encodedBytes, point);
        }
        await fdPtau.write(affine);

        writtenPoints += chunkPoints;
        position += chunkPoints * section.compressedBytes;
    }

    position += (section.totalPoints - section.keptPoints) * section.compressedBytes;
    await binFileUtils.endWriteSection(fdPtau);
    return position;
}

function readBufferSlice(fd, position, size) {
    const buffer = Buffer.allocUnsafe(size);
    let offset = 0;
    while (offset < size) {
        const bytesRead = fs.readSync(fd, buffer, offset, size - offset, position + offset);
        if (bytesRead === 0) {
            throw new Error(`Unexpected EOF while reading response at offset ${position + offset}`);
        }
        offset += bytesRead;
    }
...
async function generateSetupArtifacts(constraintCount, manifest) {
    ...
    await createTruncatedPhase1PtauFromDuskResponse(responsePath, rawPhase1Ptau, power);
