...
- $\mathcal{N}\subseteq\mathrm{ForkIds}\times\mathrm{StateIndices}\times\mathrm{AppStorageAddrs}\times\mathrm{ProposedStateRoots}$
  ...
  - Getter: $\mathrm{getProposedStateFork}:\mathrm{StateIndices}\to\mathcal{P}(\mathrm{ForkIds})$, where $\mathrm{getProposedStateFork}(t):=\{f\in\mathrm{ForkIds}\mid \exists s\in\mathrm{AppStorageAddrs},\ \exists r\in\mathrm{ProposedStateRoots},\ (f,t,s,r)\in\mathcal{N}\}$
...
- $\widetilde{\mathcal{N}}:=\bigcup_{c\in\mathrm{ChannelIds}}\left(\{c\}\times\mathcal{N}_c\right)$
  ...
  - Getter: $\mathrm{getChannelUnverifiedForks}:\{(c,t)\mid c\in\mathrm{ChannelIds}\ \wedge\ t\in\mathrm{StateIndices}_c\}\to\mathcal{P}(\mathrm{ForkIds}_c)$, where $\mathrm{getChannelUnverifiedForks}(c,t):=\mathrm{getProposedStateFork}_c(t)$
