...
This example now uses static `tokamak-ch-tx` launch inputs for the `packages/apps/private-state` DApp across `mintNotes1`, `mintNotes2`, `mintNotes3`, `mintNotes4`, `mintNotes5`, and `mintNotes6`.
...
1. Regenerate the static launch inputs from the integrating repository's `packages/apps/private-state` deployment flow.
