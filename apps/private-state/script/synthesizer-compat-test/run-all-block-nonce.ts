...
import { PRIVATE_STATE_SYNTH_COMPAT_FUNCTIONS, runPrivateStateSynthesizerBlockNonceCompatTest } from './common.ts';
...
for (let index = 0; index < PRIVATE_STATE_SYNTH_COMPAT_FUNCTIONS.length; index += 1) {
  ...
  await runPrivateStateSynthesizerBlockNonceCompatTest(functionName, effectiveArgs);
}
